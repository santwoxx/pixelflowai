  ...
  try {
    ...
    if (pixelJitter || grainApplied) {
      ...
      for (let i = 0; i < len; i += channels) {
        ...
        rawPixels[i] = Math.min(255, Math.max(0, rawPixels[i] + noise + subJitter));
        if (channels >= 3) {
          rawPixels[i+1] = Math.min(255, Math.max(0, rawPixels[i+1] + noise - subJitter));
          rawPixels[i+2] = Math.min(255, Math.max(0, rawPixels[i+2] + noise + subJitter));
        }
